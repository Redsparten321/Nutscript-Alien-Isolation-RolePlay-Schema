...
function SWEP:Think()
	...
	if self.HoldedFlareLifeTime < CurTime() and self.IsOn and SERVER then
		...
		if self.Owner.FireLoopSound then self.Owner.FireLoopSound:Stop() end
		self:SendWeaponAnim( ACT_VM_HOLSTER )
		timer.Simple( 1, function() if IsValid( self ) then if self.Owner:GetAmmoCount( 'ai_flares' ) <= 0 then return end self:SendWeaponAnim( ACT_VM_DRAW ) end end )
	end
	...
	if CLIENT and self:GetNWBool( 'IsON' ) then
		...
		self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * ( 20 - 23 * tprec ) + ( self.Owner:EyeAngles():Right() * ( 7 - ( 6 * prec ) ) ) + self.Owner:EyeAngles():Up() * ( 1.3 - ( 7 * prec ) ), 
		ang, self )
	end
	...
	if SERVER and self:GetNWBool( 'IsON' ) then
		...
		ang = Angle( ang.p, ang.y, ang.r - 45 )
		ParticleEffect( 'alien_flare_fire', 
		self.Owner:GetShootPos() - self.Owner:EyeAngles():Forward() * 10 + self.Owner:EyeAngles():Right() * 22 + self.Owner:EyeAngles():Up() * 1.3, 
		ang, self )
	end
	...
	if self.ThrowingTime < CurTime() and self.Throwing and self.NextThrow < CurTime() then
		...
		self.Throwing = false
		self.Owner:ViewPunch( Angle( 0, 10 + 10 * ( self.ThrowingForce / 1000 ), -10 - 10 * ( self.ThrowingForce / 1000 ) ) )
		timer.Simple( 0.2, function() if IsValid( self ) then
			if self.Owner.FireLoopSound then self.Owner.FireLoopSound:Stop() end
				self.IsOn = false
				self:SetNWBool( 'IsON', false )
				local flare = ents.Create( 'prop_physics' )
				flare:SetPos( self.Owner:GetShootPos() + self.Owner:EyeAngles():Right() * 5 )
				flare:SetAngles( self.Owner:EyeAngles() - Angle( 0, 0, 15 ) )
				flare:SetModel( 'models/weapons/w_flare_nocap.mdl' )
				flare:Spawn()
				flare:Activate()
				flare:GetPhysicsObject():SetVelocity( self.Owner:GetAimVector() * self.ThrowingForce )
				...
				timer.Simple( 0.3, function() if IsValid( self ) then if self.Owner:GetAmmoCount( 'ai_flares' ) <= 0 then return end self:SendWeaponAnim( ACT_VM_DRAW ) end end )
			end 
		end )
		self:SetHoldType( 'slam' )
	...
	if self.ThrowingTime > CurTime() and self.Throwing and self.Owner:KeyDown( IN_ATTACK ) then
...
function SWEP:PrimaryAttack()
	...
	self.Owner.FireLoopSound = CreateSound( self, 'weapons/flare_loop.wav' )
	timer.Simple( 1.5, function() if self.Owner.FireLoopSound then self.Owner.FireLoopSound:Play() end end )
	self:SetNextPrimaryFire( CurTime() + 1 )
end
...
function SWEP:Holster()
	if self:GetNWBool( 'IsON' ) and SERVER then
		local flare = ents.Create( 'prop_physics' )
		flare:SetPos( self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * 5 )
		flare:SetModel( 'models/weapons/w_flare_nocap.mdl' )
		flare:Spawn()
		flare:Activate()
		if self.Owner.FireLoopSound then self.Owner.FireLoopSound:Stop() end
